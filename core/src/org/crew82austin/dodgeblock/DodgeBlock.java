...
public class DodgeBlock extends Game implements InputProcessor {
  ...
  public void render() {
    super.render();
    /*
     * deltaT = Gdx.graphics.getDeltaTime(); Gdx.gl.glClearColor(0, 0, 0, 1);
     * Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
     * 
     * 
     * runner.update(deltaT); players = runner.getCurrentPlayers();
     * batch.begin(); for(int c = 0; c < players.size(); c++){ float px =
     * players.get(c).getX() + players.get(c).getSize() + 2; float py =
     * players.get(c).getY() + players.get(c).getSize() + 2; font.draw(batch,
     * players.get(c).getName(), px, py); } if(runner.getWinner() != null &&
     * runner.getState() == GameRunner.State.FINISHED){ font.draw(batch,
     * "The winner is "+runner.getWinner().getName(), 5,
     * Gdx.graphics.getHeight() - 20); } batch.end();
     * 
     * rend.setAutoShapeType(false); rend.begin(ShapeType.Filled); for(int b =
     * 0; b < players.size(); b++){ players.get(b).draw(rend); } rend.end();
     * 
     * runner.updateLocal(localPlayer1); if(TOWP)
     ...
     * 
     * for(int a = 0; a < players.size(); a++){ players.get(a).update(deltaT); }
     */
  }
